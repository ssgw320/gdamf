    ...
    for i, (x, y) in enumerate(zip(x_all, y_all)):
        ...
        if i+1 < len(x_all):
            ...
            for idx, (_x, _y) in enumerate(zip(x_next, y_next)):
                ...
                if (r.item() > 0.5) & (sample == 1) & (_y != y_pred.item()) & (budget > cost[i]):
                    # query
                    masked_y_all[i+1][idx] = _y
                    queried_num[i] += 1
                    budget -= cost[i]
                    # udate classifier
                    train_idx = np.argwhere(masked_y_all[i+1] != -1).flatten()
                    model.train()
...
def AuxSelfTrain(x_all, y_all, num_inter, hidden_dim=32, n_epochs=100, weight_decay=1e-3):
    """
    Zhang proposed
    Gradual Domain Adaptation via Self-Training of Auxiliary Models
    https://arxiv.org/abs/2106.09890
