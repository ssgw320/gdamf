...
if __name__ == '__main__':
    ...
    for i, budget in enumerate(budgets):
        for r in range(rep):
            ...
            pd.to_pickle({'budgets': budgets, 'acc': gdamf}, f'{key}_gdamf.pkl')
            query[i, r, :] = queried_num
            pd.to_pickle({'budgets': budgets, 'query': query}, f'query_{key}.pkl')
            ...
            pd.to_pickle({'budgets': budgets, 'acc': gdamf_rnd}, f'{key}_gdamf-rnd.pkl')
            ...
            pd.to_pickle({'budgets': budgets, 'acc': gdamf_direct}, f'{key}_gdamf-direct.pkl')
            ...
            pd.to_pickle({'budgets': budgets, 'acc': gdamf_abl}, f'{key}_gdamf-abl.pkl')
            ...
            pd.to_pickle({'budgets': budgets, 'acc': gdamf_ws}, f'{key}_gdamf-ws.pkl')
            query_ws[i, r, :] = queried_num
            pd.to_pickle({'budgets': budgets, 'query': query_ws}, f'query-ws_{key}.pkl')
            # target only
            all_model, _ = G.TargetOnly(x_subset, y_subset, num_init_labels, budget, cost)
            to[i, r] = G.calc_accuracy(all_model[-1], x_eval, y_eval)
            pd.to_pickle({'budgets': budgets, 'acc': to}, f'{key}_targetonly.pkl')
            # target only with AL
            ...
            pd.to_pickle({'budgets': budgets, 'acc': to_al}, f'{key}_targetonly-al.pkl')
